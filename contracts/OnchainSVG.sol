...
/// @dev    Foundry-friendly: errors are surfaced with public selectors so forge
///         can `vm.expectRevert` against them in tests. The 24 KiB SVG cap is
///         enforced onchain to keep mints inside Pharos block gas limits.
contract OnchainSVG is ERC721, ERC721URIStorage, Ownable {
    ...
    /// @param initialOwner     Address that owns admin rights (defaults to msg.sender)
    constructor(
    ...
    /// @notice Mint a token with onchain metadata (name + description).
    ...
    function _buildMetadataJSON(
        ...
        string memory tokenName =
            bytes(data.name).length == 0 ? string.concat("Onchain SVG #", tokenId.toString()) : data.name;

        string memory tokenDescription =
            bytes(data.description).length == 0 ? "Fully onchain SVG minted with SVGM on Pharos." : data.description;

        return string.concat(
            '{"name":"',
            _escape(tokenName),
            '","description":"',
            _escape(tokenDescription),
    ...
    function _enforceSize(
        string memory svg
    ) internal pure {
        uint256 max = 24_576; // 24 KiB
        uint256 size = bytes(svg).length;
        if (size > max) revert SVGTooLarge(size, max);
    }

    /// @dev Defense-in-depth: reject SVGs containing <script> tags, javascript: URIs, or `onerror=`.
    function _enforceSafe(
        string memory svg
    ) internal pure {
    ...
    /// @dev Non-cryptographic 8-hex fingerprint for the event log.
    function _hashOf(
        string memory s
